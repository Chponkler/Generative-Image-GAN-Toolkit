...
dataset_path      = "/content/drive/MyDrive/your_dataset"  # path to your dataset (путь к вашему датасету)
batch_size        = 64
...
# 3. Data transformations with augmentations (Трансформации данных с аугментациями)
transform = transforms.Compose([
    ...
    transforms.RandomAffine(
        degrees=0,                                             # already did RandomRotation (уже сделали RandomRotation)
        translate=(0.1, 0.1),                                  # translate up to 10% (смещение до 10%)
        scale=(0.8, 1.2),                                      # scale between 80–120% (масштаб 80–120%)
    ...
    transforms.Normalize([0.5]*3, [0.5]*3),                    # Normalize to [-1,1] (Нормализация до [-1,1])
])

# 4. Dataset and DataLoader (Набор данных и загрузчик)
dataset = datasets.ImageFolder(dataset_path, transform=transform)
loader  = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True)

# 5. Define dynamic Generator and Discriminator (Определение динамических Generator и Discriminator)
class Generator(nn.Module):
    def __init__(self):
        ...
        mult = 2**(n-3)
        layers = []
        # Initial block: nz -> ngf*mult, output 4×4 (Начальный блок)
        layers += [
        ...
        c = ngf*mult
        # Upsampling blocks until reach image_size/2 (Апсемплинг до image_size/2)
        for _ in range(n-3):
            layers += [
            ...
            c //= 2
        # Final block: output channels=3 (Финальный блок: 3 канала)
        layers += [
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        layers = []
        c = ndf
        # Initial block: 3×image_size -> half size (Начальный блок)
        layers += [
            nn.Conv2d(3, c, 4, 2, 1, bias=False),
            nn.LeakyReLU(0.2, inplace=True)
        ]
        # Downsampling blocks until reach 4×4 (Даунсемплинг до 4×4)
        for _ in range(n-3):
            layers += [
            ...
            c *= 2
        # Final block: 1×1 output → probability (Финальный блок: 1×1 → вероятность)
        layers += [
...
# 6. Initialize models, loss and optimizers (Инициализация моделей, loss и оптимайзеров)
...
# 7. Load checkpoint if resuming (Загрузка чекпоинта при возобновлении)
start_epoch = 1
...
fixed_noise = torch.randn(64, nz, 1, 1, device=device)  # Fixed noise for visuals (Фиксированный шум для визуализации)

# 8. Training loop (Цикл обучения)
for epoch in range(start_epoch, num_epochs+1):
    pbar = tqdm(loader, desc=f"Epoch {epoch}/{num_epochs}")
    for real, _ in pbar:
        real = real.to(device)
        b = real.size(0)
        real_lbl = torch.ones(b, device=device)   # Real label = 1 (Метка реальных = 1)
        fake_lbl = torch.zeros(b, device=device)  # Fake label = 0 (Метка фейковых = 0)
    ...
    if epoch % save_every == 0 or epoch == num_epochs:
        ...
        print(f"Saved checkpoint: {path} (Чекпоинт сохранён: {path})")

    # Visualization (Визуализация)
    with torch.no_grad():
        imgs = G(fixed_noise).cpu()
    grid = utils.make_grid(imgs, padding=2, normalize=True)
    torchvision.utils.save_image(grid, f"/content/fake_{epoch}.png")
    display(grid)
